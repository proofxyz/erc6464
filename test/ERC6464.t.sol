...
contract TestableReferenceERC6464 is ERC6464, ITestableReferenceERC6464 {
    ...
    function transferFrom(
        ...
        require(
            isExplicitlyApprovedFor(msg.sender, tokenId) ||
...
contract ERC6464Test is Test, IERC6464Events {
    ...
    function _testSetExplicitApproval(ExplicitApprovalTest memory _testValues)
        public
    {
        ...
        vm.expectRevert(
            abi.encodeWithSignature(
                "NotOwner(address,uint256)",
                _testValues.operator,
                0
            )
        );
        vm.prank(_testValues.operator);
        if (_testValues.tokenId < 1) {
            erc6464.setExplicitApproval(
                _testValues.operator,
                _testValues.tokenId,
                _testValues.approve
            );
        ...
        assertEq(
            erc6464.isExplicitlyApprovedFor(
                _testValues.operator,
                _testValues.tokenId
            ),
            _testValues.approve
        ...
        assertEq(
            erc6464.isApprovedFor(_testValues.operator, _testValues.tokenId),
            _testValues.approve
        );
        ...
        if (_testValues.tokenId > 1) {
            ...
            erc6464.revokeAllExplicitApprovals(_testValues.tokenId);
            assertEq(
                erc6464.isExplicitlyApprovedFor(_testValues.operator, 0),
                _testValues.approve
            );
            vm.stopPrank();
    ...
    function testExplicitApproval(
        ...
        _testSetExplicitApproval(
            ExplicitApprovalTest(_owner, _operator, _approve, 0)
        );
    }

    function testMultipleExplicitApprovals(
        address _owner,
        address _operator,
        bool _approve,
        uint256 _tokenId
        ...
        vm.assume(_owner != _operator);
        _testSetExplicitApproval(
            ExplicitApprovalTest(_owner, _operator, _approve, _tokenId)
        );
    }
}
...
contract ReferenceERC6464Test is ERC6464Test {
    constructor()
        ERC6464Test(new TestableReferenceERC6464("Non Fungible Token", "NFT"))
    {}
}
